        ...
        for (const arg of argSpec) {
            switch (arg.type) {
                case 'm':
                    ...
                    break;
                case 'o':
                    args.push(parser.GetBrackets(name) || '-NoValue-');
                    break;
                case 'O':
                    args.push(parser.GetBrackets(name) || arg.default);
                    break;
                case 's':
                    args.push(parser.GetStar() ? '\\BooleanTrue' : '\\BooleanFalse');
                    break;
                case 't': {
                    ...
                    args.push(tok ? '\\BooleanTrue' : '\\BooleanFalse');
                    break;
                }
            }
        }
...
function GetCsNameArgument(parser, name) {
    ...
    if (!cs.match(/^(.|[a-z]+)$/i)) {
        throw new TexError('IllegalControlSequenceName', 'First argument of \'%1\' must be a command.', parser.currentCS);
    }
    return cs;
}
...
xparseMethods.documentCmd = function (parser, name) {
